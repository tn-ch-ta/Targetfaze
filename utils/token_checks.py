    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if len(top_holders) >= 2:
                ...
                if second_pct >= 3.7:
                    logger.warning(f"[❌TOP HOLDERS] {mint_address} 2nd holder pct={second_pct:.4f}% >= 3.7% — FAIL")
                    return None
                logger.info(f"[✅TOP HOLDERS] {mint_address} 2nd holder pct={second_pct:.4f}% < 3.7% — PASS")
            else:
